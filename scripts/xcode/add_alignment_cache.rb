...
puts "🔧 Adding AlignmentCache files to Listen2 project..."
...
# Add AlignmentCache.swift to app target
alignment_cache_path = 'Listen2/Services/TTS/AlignmentCache.swift'
existing_alignment_cache = project.files.find { |f| f.path&.include?('AlignmentCache.swift') }

if existing_alignment_cache
  puts "⏭️  AlignmentCache.swift already in project"
else
  file_ref = tts_group.new_file(alignment_cache_path)
  app_target.source_build_phase.add_file_reference(file_ref)
  puts "✅ Added AlignmentCache.swift to Listen2 target"
end

# Add AlignmentCacheTests.swift to test target
test_file_path = 'Listen2Tests/Services/AlignmentCacheTests.swift'
existing_test = project.files.find { |f| f.path&.include?('AlignmentCacheTests.swift') }

if existing_test
  puts "⏭️  AlignmentCacheTests.swift already in project"
else
  ...
  puts "✅ Added AlignmentCacheTests.swift to Listen2Tests target"
end

# Verify files exist on disk
files_to_check = [
  ['AlignmentCache.swift', alignment_cache_path],
  ['AlignmentCacheTests.swift', test_file_path]
]
