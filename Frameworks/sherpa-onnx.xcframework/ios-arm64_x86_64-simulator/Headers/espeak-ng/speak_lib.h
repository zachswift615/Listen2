...
*/
         ...
         /********************************/
         /*  Normalized Text Exposure    */
         /********************************/

#ifdef __cplusplus
extern "C"
#endif
ESPEAK_API const char* espeak_GetNormalizedText(void);
/* Get the normalized text from the last translation operation.
   This includes text expansion (Dr. -> Doctor, 123 -> one hundred twenty three)

   Note: Currently returns the clause text as processed by ReadClause.
   Future enhancements will capture actual text transformations during word processing.

   Return: Pointer to normalized text buffer (valid until next translation call)
           NULL if no translation has been performed yet
*/

#ifdef __cplusplus
extern "C"
#endif
ESPEAK_API int espeak_GetCharacterMapping(int (*map_array)[2], int max_entries);
/* Get the character position mapping between original and normalized text.

   map_array: Output array where each entry is [original_pos, normalized_pos]
   max_entries: Maximum number of entries to write to map_array

   Return: Number of entries written to map_array
           0 if no translation has been performed yet
*/
